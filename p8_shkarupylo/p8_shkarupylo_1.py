...
prog = re.compile(r'BodyPart:\d+-\(| score=|\[|]|,|\)')
result = [float(i) for i in prog.split(pose_estimation) if i != '']
points = [result[i] for i in range(len(result)) if i % 3 != 2]
score = [result[i] for i in range(len(result)) if i % 3 == 2]
